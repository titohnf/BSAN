...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (user) {
      try {
        sessionStorage.setItem("auth", JSON.stringify({ username: user.username, role: user.role }))
      } catch {}
      router.replace("/")
    } else {
    ...
    <main className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        ...
        <p className="text-center text-xs text-gray-400 mt-6">
          Dinas Pendidikan — Sistem Informasi POKJA
        </p>
      </div>
    </main>
  )
...
mkdirSync(join("/vercel/share/v0-project/app/login"), { recursive: true })
writeFileSync(join("/vercel/share/v0-project/app/login/page.tsx"), loginPage, "utf8")
